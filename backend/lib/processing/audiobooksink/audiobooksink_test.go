...
func TestAudiobookSink(t *testing.T) {
	...
	sink := audiobooksink.NewAudiobookSink(&mockRepo)
	sinkPipelineComponent := pipeline.NewPipelineComponent(sink)
	context, cancel := context.WithCancel(context.Background())

	inputChan := make(chan models.AudiobookProcessed)
	...
	go func() {
		...
		case <-doneConsumerChan:
			return
		case <-sinkPipelineComponent.OutputChan:
			outputReceived = true
		}
	}()

	sinkPipelineComponent.Start(context, inputChan, donePipelineChan)
	inputChan <- audiobook
